...
class DotAttention(AttentionBase):
    def forward(self,
        ...
        encoder_tensor = torch.sum(torch.mul(encoder_hidden, decoder_hidden.unsqueeze(1)), -1)
        encoder_tensor[encoder_mask] = float("-inf")

        distribution = torch.exp(nn.functional.log_softmax(encoder_tensor, dim=1))
