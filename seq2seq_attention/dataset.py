...
def preprocess(
    ...
    for word in raw_trg_sentence:
        try:
            trg_sentence.append(trg_word2idx[word])
        except:
            trg_sentence.append(UNK)

    if len(src_sentence) > max_len:
        src_sentence = src_sentence[:max_len]

    if len(trg_sentence) >= max_len-2:
        trg_sentence = trg_sentence[:max_len-2]
...
def bucketed_batch_indices(
    ...
    src_len_min = min(sentence_length, key = lambda x:x[0])[0]
    trg_len_min = min(sentence_length, key = lambda x:x[1])[1]

    for idx, (src_len, trg_len) in enumerate(sentence_length):
        src = (src_len - src_len_min + 1) // (max_pad_len + 1)
        trg = (trg_len - trg_len_min + 1) // (max_pad_len + 1)
        batch_map[(src,trg)].append(idx)
...
def collate_fn(
    ...
    src_sentences = []
    trg_sentences = []
    for src_sentence, trg_sentence in batched_samples:
        src_sentences.append(torch.tensor(src_sentence))
        trg_sentences.append(torch.tensor(trg_sentence))

    src_sentences = torch.nn.utils.rnn.pad_sequence(src_sentences, batch_first=True)
    trg_sentences = torch.nn.utils.rnn.pad_sequence(trg_sentences, batch_first=True)
